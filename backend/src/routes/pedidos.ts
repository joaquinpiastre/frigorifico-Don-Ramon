...
pedidosRouter.post('/pedidos', requireAuth, requireRol('admin'), async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      await client.query(
        `insert into pedido_items (pedido_id, producto_id, res_id, cantidad, precio, garron, tropa)
         values ($1, $2, $3, $4, $5, $6, $7)`,
        [pedidoId, item.productoId, item.resId ?? null, item.cantidad, item.precio, item.garron ?? null, item.tropa ?? null]
      );
    }
...
pedidosRouter.get('/pedidos/:id', requireAuth, async (req, res) => {
  ...
  const items = await pool.query(
    `select pi.id, pi.producto_id as "productoId", pr.nombre as "productoNombre",
            pi.res_id as "resId", r.cor, pi.cantidad, pi.precio, pi.garron, pi.tropa, pi.entregado
...
async function descontarStockItem(
  ...
  const disponibleTotal = lotesStock.reduce((acc, r) => acc + Number(r.cantidadDisponible), 0);
  if (disponibleTotal < restante) {
    throw Object.assign(
      new Error(`No hay suficiente stock del producto ${item.productoId} (disponible: ${disponibleTotal}).`),
      { status: 400 }
    );
  }
  for (const lote of lotesStock) {
    ...
    await client.query('update items_stock set cantidad_disponible = cantidad_disponible - $1 where id = $2', [
      aDescontar,
      lote.id,
    ]);
    restante -= aDescontar;
...
async function cambiarEstadoPedido(
  ...
  try {
    ...
    const { rows } = await client.query('select estado from pedidos where id = $1 for update', [pedidoId]);
    if (rows.length === 0) {
      await client.query('ROLLBACK');
    ...
    if (rows[0].estado !== estadoActualEsperado) {
      ...
      return {
        status: 409,
        body: { error: `El pedido debe estar en estado "${estadoActualEsperado}" (actual: "${rows[0].estado}").` },
      };
    }
    ...
    if (estadoNuevo === 'entregado') {
      await client.query('update pedido_items set entregado = true, entregado_en = now() where pedido_id = $1', [
        pedidoId,
      ]);
    }

    await client.query(`update pedidos set estado = $2, ${columnaFecha} = now() where id = $1`, [
      pedidoId,
      estadoNuevo,
    ]);
...
pedidosRouter.patch('/pedidos/:id/armar', requireAuth, requireRol('operador', 'admin'), async (req, res) => {
  const id = Number(req.params.id);
  const { status, body } = await cambiarEstadoPedido(id, 'pendiente', 'armado', 'armado_en', true);
  res.status(status).json(body);
});

// PATCH /pedidos/:id/cargar — operador o repartidor confirman que subió a la camioneta
pedidosRouter.patch('/pedidos/:id/cargar', requireAuth, requireRol('operador', 'repartidor', 'admin'), async (req, res) => {
  const id = Number(req.params.id);
  const { status, body } = await cambiarEstadoPedido(id, 'armado', 'cargado', 'cargado_en', false);
  res.status(status).json(body);
});

// PATCH /pedidos/:id/entregar — el repartidor marca el pedido como entregado al cliente
pedidosRouter.patch('/pedidos/:id/entregar', requireAuth, requireRol('repartidor', 'admin'), async (req, res) => {
  const id = Number(req.params.id);
  const { status, body } = await cambiarEstadoPedido(id, 'cargado', 'entregado', 'entregado_en', false);
  res.status(status).json(body);
});
