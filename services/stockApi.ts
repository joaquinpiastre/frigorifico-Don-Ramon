...
export async function actualizarLoteApi(
  id: number,
  input: {
    numeroTropa?: string;
    dte?: string;
  ...
  const data = await apiRequest<{ lote: LoteIngreso }>(`/admin/lotes/${id}`, {
    method: "PATCH",
    body: JSON.stringify(input),
  });
  return data.lote;
...
export async function buscarResPorCodigoApi(
  ...
  try {
    const data = await apiRequest<{ res: Res }>(
      `/admin/reses/${encodeURIComponent(codigo)}`,
    );
    return data.res;
  } catch {
    return null;
