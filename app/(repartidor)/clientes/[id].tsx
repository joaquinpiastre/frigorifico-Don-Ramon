...
export default function ClienteDetalleRepartidor() {
  ...
  const cargar = useCallback(() => {
    ...
    obtenerClienteApi(clienteId)
      .then((data) => {
        ...
        setCondicionIva(data.cliente.condicionIva ?? null);
  ...
  const guardarEdicion = async () => {
    if (!nombre.trim()) {
      showAlert("Cliente", "El nombre no puede estar vacío.");
      return;
    }
    setGuardandoEdicion(true);
    try {
      await actualizarClienteApi(clienteId, {
        nombre: nombre.trim(),
              ...
              {CONDICIONES.map((c) => (
                ...
                >
                  <Text
                    style={[
                      styles.chipTexto,
                      condicionIva === c && styles.chipTextoActivo,
                    ]}
                  >
      ...
      {ventas.length === 0 ? (
        ...
        ventas.map((v) => (
          <Pressable
            ...
            onPress={() =>
              v.origen === "pedido"
                ? router.push(`/(repartidor)/pedidos/${v.id}/remito`)
                : showAlert(
                    "Remito",
