...
import { editarPedidoApi, obtenerPedidoApi } from "@/services/pedidosApi";
...
export default function EditarPedidoOperador() {
      ...
      .then((data) => {
        ...
        setLineas(
          data.items.map((item) => ({
            ...
            precio: item.precio,
            garron: item.garron ?? undefined,
            tropa: item.tropa ?? undefined,
            nota: item.nota ?? undefined,
            resId: item.resId ?? undefined,
  ...
  useEffect(() => {
    if (!pedido || repartidorSeleccionado) return;
    const actual = repartidores.find((r) => r.id === pedido.repartidor);
    if (actual) setRepartidorSeleccionado(actual);
  }, [pedido, repartidores, repartidorSeleccionado]);
  ...
  const guardarCambios = async () => {
    if (lineas.length === 0) {
      showAlert("Pedido", "El pedido debe tener al menos una línea.");
      return;
    }
    setGuardando(true);
    try {
      await editarPedidoApi(pedidoId, {
      ...
      showAlert(
        "Pedido",
        e instanceof Error ? e.message : "No se pudo guardar el pedido.",
      );
    } finally {
      setGuardando(false);
    }
  ...
  if (cargando) {
    return (
      <Screen title="Editar pedido" scrollable>
  ...
  if (!pedido) {
    return (
      <Screen title="Editar pedido" scrollable>
        <Text style={styles.sub}>No se encontró el pedido.</Text>
      </Screen>
    );
  }
    ...
    return (
      <Screen title="Editar pedido" scrollable>
        <Text style={styles.sub}>
          Este pedido ya fue armado y no se puede editar.
        </Text>
      </Screen>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.card}>
        ...
        {lineas.length === 0 ? (
          <Text style={styles.sub}>Sin líneas — agregá al menos una.</Text>
        ) : (
          lineas.map((l, i) => (
